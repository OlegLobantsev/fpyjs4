...
function calc(answer) {
    return new Promise((resolve, reject) => {
        if (answer === "q") {
            ...
            resolve(`Введено некорректное значение. Попытка ${count}`);
        } else if (+answer < num) {
            count += 1;
            resolve(`Ваше число меньше, чем загадано. Играем дальше! Попытка ${count}`);
        } else if (+answer > num) {
            count += 1;
            resolve(`Ваше число больше, чем загадано. Попытка ${count}`);
        } else if (+answer === num) {
            inp.close();
